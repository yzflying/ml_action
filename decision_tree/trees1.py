...
def cal_shannonent(dataset):  #计算熵：数据集各输出Y的可能性。。。之和
    ...
    for key in laber_cnt:
        pro = laber_cnt[key]/num_ent              # 计算标签key的概率
        shannonent = shannonent - pro * math.log(pro, 2)    # 计算标签key的熵，并求和
    return shannonent
...
def split_dataset(dataset, fec, value):
    ...
    返回数据集：删除特征列，删除特征列的值非指定value的行(留下属性fec值为value的样本)
    """
    ret_dataset = []
    for ent in dataset:
        if ent[fec] == value:
...
def choose_better_fec(dataset):
    ...
    for i in range(num_fec):      #对于每一个特征i
        ...
        # print(fec_list)
        # print(fec_uniq)
        shannonent = 0    #对每个特征，记得把shannonent归零
...
def file_to_matrix(filename):     #根据filename输出：5维数据集dataSet
    data = open(filename)
    arraylines = data.readlines()  #按行读取，arraylines为list变量，长度为行数
    m = len(arraylines)
    dataSet = np.zeros((m, 5), dtype=np.object)     #创造0矩阵，存储5维数据集(数据为字符串)
    index = 0
    for line in arraylines:
        line = line.split()       #line为该行的元素列表
        dataSet[index, :] = line[0:5]
        index = index + 1
    return dataSet


filename = "lenses.txt"
dataset = file_to_matrix(filename)       #文件转换为矩阵（包含输入和输出Y）
labels = ['age', 'prescript', 'astigmatic', 'tearrate']   #特征名称
print(dataset)
# dataset[:, [0, 3]] = dataset[:, [3, 0]]       #dataset的第0列和第3列交换
# print(dataset)
